                ...
                >
                  {filteredButtons.map(button => (
                    <ActionButtonList
                      ...
                      disabledReason={getDisabledReason?.(button.id) || null}
                    />
                  ))}
                </Box>
              )
...
const ActionButtonList: React.FC<{
...
}> = ({ button, isFavorite, onActionClick, onFavoriteToggle, disabledReason }) => {
  ...
  return (
    <Box
      ...
      onClick={() => {
        if (!disabled) onActionClick(button.id);
      }}
      title={disabledReason || button.tooltip || button.label}
    >
      <Box
        sx={{
          display: 'flex',
