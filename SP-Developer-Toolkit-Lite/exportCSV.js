...
(function () {
  ...
  params = readParams();
  if (!params) {
    console.error("SP Developer Toolkit Lite – Missing params.");
    return;
  }
  // -----------------------------
  // Helpers
  ...
  function reportDone(success, message, stopReason) {
    if (!success) {
      ...
      try { alert("SP Developer Toolkit Lite – Export failed:\n\n" + message); } catch (_) {}
    }
    window.postMessage(
  ...
  // Working view name (used for owssvr download). Lite: expect this view to already exist.
  var RPC_VIEW_TITLE = String(params.vt || "RPC");
  var LITE_READ_ONLY = true;  // No PATCH/POST: use existing RPC view as-is, single page per export
  ...
  async function getDefaultViewId() {
    var accept = { "Accept": "application/json;odata=nometadata" };
    var lb = listBaseUrl();
    var r = await fetch(lb + "/DefaultView?$select=Id", { credentials: "include", headers: accept });
    if (!r.ok) return null;
    var j = await r.json();
    var id = j.Id != null ? j.Id : (j.d && j.d.Id);
    return id ? normalizeGuid(id) : null;
  }

  // Lite: use any existing shared view named RPC (e.g. RecursiveAll 1). No Scope/ViewQuery validation.
  async function getRpcViewIdOnly() {
    var accept = { "Accept": "application/json;odata=nometadata" };
    var lb = listBaseUrl();
    var viewsResp = await fetch(lb + "/views?$select=Id,Title,PersonalView", { credentials: "include", headers: accept });
    if (!viewsResp.ok) {
      var t0 = "";
    ...
    var serverView = null;
    var personalView = null;
    for (var i = 0; i < views.length; i++) {
      var v = views[i];
      var viewTitle = (v.Title || "").trim();
      if (viewTitle.toUpperCase() !== RPC_VIEW_TITLE.toUpperCase()) continue;
      if (v.PersonalView === true) {
        personalView = v;
      } else {
        serverView = v;
      }
    }
    if (personalView && !serverView) {
      return { ok: false, error: "View '" + RPC_VIEW_TITLE + "' is a personal view. Create a shared view named '" + RPC_VIEW_TITLE + "' (List Settings > Views > Create view, save as shared)." };
    }
    if (!serverView) {
      return { ok: false, error: "No shared view named '" + RPC_VIEW_TITLE + "'. Create one: List Settings > Views > Name 'RPC', shared, Scope RecursiveAll. Then run export again." };
    }
    var rawId = serverView.Id != null ? serverView.Id : serverView.ID;
    if (rawId == null || rawId === "") return { ok: false, error: "RPC view has no Id." };
    var viewId = normalizeGuid(rawId);
    if (!viewId) return { ok: false, error: "RPC view Id could not be normalized." };
  ...
  function finishOwssvrExport(parsed) {
    ...
    var msg = "Done! " + rows.length.toLocaleString() + " rows exported.";
    if (parsed.usedDefaultViewFallback) msg += " (Used default view; RPC view returned 404 on this site.)";
    reportDone(true, msg);
  }
  ...
  async function exportViaOwssvrWithView(viewId, itemCount, retryCount) {
    ...
    while (pageNum < MAX_PAGES) {
      ...
      // Lite: no PATCH view query; use existing RPC view as-is (single page)

      var url = buildOwssvrUrl(viewId);
      var resp = await fetch(url, { credentials: "include", redirect: "follow" });
      if (!resp.ok && resp.status === 404) {
        if (pageNum === 1) {
          var defaultViewId = await getDefaultViewId();
          if (defaultViewId) {
            var fallbackUrl = buildOwssvrUrl(defaultViewId);
            var fallbackResp = await fetch(fallbackUrl, { credentials: "include", redirect: "follow" });
            if (fallbackResp.ok) {
              resp = fallbackResp;
              viewId = defaultViewId;
              usedDefaultViewFallback = true;
            } else {
              reportDone(false, "This site does not support the owssvr export endpoint (RPC and default view both returned 404). Use the full SPO Dev Toolkit or PowerShell. Your RPC view must match: shared, Title 'RPC', Scope = RecursiveAll (1 or 2), ViewQuery empty.");
              return;
            }
          } else {
            reportDone(false, "owssvr returned 404. RPC view must match: shared, Title 'RPC', Scope = RecursiveAll (1 or 2), no filter. Ensure you're on the list/library page. Some sites don't support owssvr—use the full SPO Dev Toolkit or PowerShell.");
            return;
          }
        } else {
          reportDone(false, owssvrIncompleteMessage("owssvr returned 404 (view may no longer be available).", allRows.length));
          return;
        }
      }
      if (!resp.ok) {
        reportDone(false, owssvrIncompleteMessage("owssvr.dll HTTP " + resp.status + ": " + resp.statusText + ".", allRows.length));
        return;
      }
    ...
    finishOwssvrExport({ rows: rowsToExport, columns: [], usedDefaultViewFallback: usedDefaultViewFallback });
  }
  ...
  (async function () {
    ...
    try {
      ...
      if (useRestForVersions) {
        ...
        var vr = await getRpcViewIdOnly();
        if (!vr.ok) {
          reportDone(false, "RPC view: " + (vr.error || "unknown"));
          return;
        }
        rpcViewId = vr.viewId;
        await sleep(500);
      ...
      console.error("SP Developer Toolkit Lite – Error:", e);
