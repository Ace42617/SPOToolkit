  ...
  fetch(listsUrl, { credentials: "include", headers: { Accept: accept } })
    ...
    .then(function (j) {
      ...
      var lists = results.map(function (item) {
        ...
        return { title: title, viewUrl: viewUrl || "", isLibrary: isLibrary, typeLabel: typeLabel, itemCount: itemCount, modified: modified };
      }).filter(function (item) { return item.viewUrl; });
      window.postMessage({ __spcsv: true, type: "SPCSVSiteListsResult", lists: lists, siteUrl: siteUrl, currentListId: currentListId || null }, "*");
    })
    .catch(function (err) {
      window.postMessage({ __spcsv: true, type: "SPCSVSiteListsResult", error: (err && err.message) || "Failed to load lists" }, "*");
    });
