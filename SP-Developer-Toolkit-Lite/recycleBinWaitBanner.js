...
(function () {
  ...
  function remainingWaitMs() {
    try {
      ...
      var started = Number(sessionStorage.getItem(TS_KEY) || "0");
      if (!started || !isFinite(started)) {
        teardown();
        return 0;
      }
      var remaining = maxWaitMs - (Date.now() - started);
      if (remaining <= 0) {
        teardown();
        return 0;
      }
  ...
  function tryPrime() {
    ...
    mount();
    window.setTimeout(teardown, remaining);
  }
