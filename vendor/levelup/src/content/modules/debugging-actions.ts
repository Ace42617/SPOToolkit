...
export class DebuggingActions {
  ...
  /**
   * Disable Dark Mode by removing themeoption=darkmode from the flags URL param(s).
   * Leaves other flags intact (unlike clearFlags).
   */
  disableDarkMode() {
    const url = new URL(window.location.href);
    const isDarkFlag = (value: string) =>
      decodeURIComponent(value).replace(/\s+/g, '').toLowerCase() === 'themeoption=darkmode';

    const flags = url.searchParams.getAll('flags');
    const remaining = flags.filter(f => !isDarkFlag(f));
    const changed = remaining.length !== flags.length;

    if (!changed && !/flags=themeoption(%3[Dd]|=)darkmode/i.test(window.location.href)) {
      return;
    }

    url.searchParams.delete('flags');
    for (const f of remaining) {
      url.searchParams.append('flags', f);
    }
