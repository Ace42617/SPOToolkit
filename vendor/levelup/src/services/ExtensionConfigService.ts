...
export class ExtensionConfigService {
  ...
  /**
   * Keep in sync when the popout header (or another surface) updates display mode.
   */
  private static bindStorageListener(): void {
    if (this.storageListenerBound) return;
    if (typeof chrome === 'undefined' || !chrome.storage?.onChanged) return;
    this.storageListenerBound = true;
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area !== 'local' || !changes[STORAGE_KEY]) return;
      const next = changes[STORAGE_KEY].newValue as Partial<ExtensionConfig> | undefined;
