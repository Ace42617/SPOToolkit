...
export function parseViewQueryParts(viewQuery) {
  ...
  while ((m = condRegex.exec(q)) !== null) {
    const typeM = /\bType\s*=\s*"([^"]*)"/i.exec(m[3] || "");
    filters.push({
      op: m[1],
      field: m[2],
  ...
  const unaryRegex = /<(IsNull|IsNotNull)\s*>\s*<FieldRef\b[^>]*\bName="([^"]+)"/gi;
...
/** Fallback decoder used by parseViewQueryParts (injected twin lives in getViewsData.js). */
function camlValueInnerToEditorFallback(inner) {
  const s = String(inner == null ? "" : inner).trim();
  const todayOff = /^<Today\b[^>]*\bOffsetDays="(-?\d+)"[^>]*\/\s*>$/i.exec(s);
  if (todayOff) {
    const n = parseInt(todayOff[1], 10);
    if (!n) return "[Today]";
