...
export function groupUniversalSearchItems(items, perGroupLimit = 18) {
  const groupsByName = new Map();
  for (const item of Array.isArray(items) ? items : []) {
    if (!groupsByName.has(item.group)) groupsByName.set(item.group, []);
    groupsByName.get(item.group).push(item);
  }

  const groups = [];
  const renderedItems = [];
  for (const [group, groupItems] of groupsByName) {
    const visibleItems = groupItems.slice(0, perGroupLimit);
    if (!visibleItems.length) continue;
    groups.push({ group, items: visibleItems });
    renderedItems.push(...visibleItems);
  }

  return { groups, renderedItems };
}
